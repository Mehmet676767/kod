...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("FIXXMUSIC.plugins" + all_module)
    LOGGER("FIXXMUSIC.plugins").info("𝐀𝐥𝐥 𝐅𝐞𝐚𝐭𝐮𝐫𝐞𝐬 𝐋𝐨𝐚𝐝𝐞𝐝 𝐁𝐚𝐛𝐲🥳...")
    await userbot.start()
    await FIXX.start()
    try:
        await FIXX.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("FIXXMUSIC").error(
    ...
    LOGGER("FIXXMUSIC").info(
        "╔═════ஜ۩۞۩ஜ════╗\n  𝗠𝗔𝗗𝗘 𝗕𝗬 ⏤𝐇𝐀𝐘𝐀𝐓𝐢 ®\n╚═════ஜ۩۞۩ஜ════╝"
    )
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("FIXXMUSIC").info("𝗦𝗧𝗢𝗣 𝐀𝐋𝐄𝐌 𝗠𝗨𝗦𝗜𝗖🎻 𝗕𝗢𝗧..")
